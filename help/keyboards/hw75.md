...
文件夹里包含如下四个文件。
```
hw75 hid-bl 1.0 to ydkb_uf2.bin
固件更新器v2.1.exe
```

操作方法是：
1. 先按Fn不放插线，键盘进入刷机模式。查看 `设置 -- 蓝牙和其他设备`，最下方`更多设备和打印机设置`里。如果刷机模式下显示的名称是 STM32F HID Bootloader，就是1.0；如果显示的是 HID-BL，就是2.x。
2. 如果确认是1.0的BL，将文件 `hw75 hid-bl 1.0 to ydkb_uf2.bin` 拖到`固件更新器v2.1`上，待烧录100%完成。如果是2.0的BL，文件我还没有准备好，稍后提供。
3. 接下来什么也不用干，等待自动完成，会自动识别为键盘然后就可以用了。当前文件里的固件是2023.1.4 DN14的版本。
4. 如果需要改按键或者要更新固件，请至 ydkb.io，选择 HW75 后，进行操作。
