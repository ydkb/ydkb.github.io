...
### 2 CSR8510 蓝牙4.0芯片的USB接收器

我使用的是Orico的一款，型号是BTA-409。这个芯片虽然是蓝牙4.0的，但是兼容性绝佳，能够长期使用稳定。直接使用Windows默认的驱动，不要使用卖家提供的驱动光盘里的。

USB的蓝牙接收器只要芯片相同，天线设计不要太烂，一般区别不大。4.0时代这一款用得非常广，同时也确实有质量参差不齐的情况。

前面内容还是19年左右写下的，时至2025年，蓝牙设备已经有不少是蓝牙5.0以上的了。因此，如果要新购买USB蓝牙接收器，不再推荐购买蓝牙4.0款的。
...
UNITEK优越者 B121A，我买到的是 Realtek(VID_0BDA&PID_A728)，这一款插上是显示 Realtek Bluetooth 5.4 Adapter。我同时使用蓝牙键盘和蓝牙鼠标，测试日常使用多日，无问题。此款USB蓝牙收发器可能还有其他方案如Barrot的。
 
Barrot BR8554。目前我还没有用过，所以暂时无法提供使用经验。仅是发现当前有出现一些蓝牙5.4的产品了，比如UGREEN绿联 CM748 和 CM749，大概率就是使用这个芯片的。欢迎反馈补充。

### 6 蓝牙6.0

绿联CM748-75073 (VID_33FA&PID_0012)，在Windows上显示为 BARROT Bluetooth 6.0 Adapter，win11系统自带有驱动。不管是连接4.0的键盘，还是5.0的键盘，使用它都是一切正常的。

注意CM748还有蓝牙5.4款的，而这里测试使用的是蓝牙6.0款的。6.0款的，在适配器外壳上也有明显的BT6.0标识。

但是，尝试了一下它连接蓝牙音箱时，播放时效果以及持续的稳定性不如Intel的蓝牙。目前是这款蓝牙6.0的产品暂时不建议选购，它的价格当前也是偏高的。

### 7 补充: Windows下只能使用一个蓝牙硬件
...
2025年补充: 新版本的windows系统下，要禁用蓝牙，有可能需要先关闭蓝牙。禁用后插上新的USB蓝牙，如果设备管理器里 `Microsoft 蓝牙 LE 枚举器` 显示有问号，重启一下系统即可。
